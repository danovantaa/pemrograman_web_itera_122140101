...
function hitungAkar(angka1, operasi){
    let hasil = 0;
    switch (operasi) {
        case "akar":
...
document.getElementById("btn-modulus").addEventListener("click", function() { 
    ...
    if (isNaN(angka1) || isNaN(angka2)) {
        ...
        const hasil = hitungKalkulator(angka1, angka2, "moduluo");
        document.getElementById("hasil-kalkulator").innerHTML = 
            `<p>Hasil: ${angka1} % ${angka2} = ${hasil.toFixed(2)}</p>`;
    }
});

document.getElementById("btn-akar2").addEventListener("click", function() {
    const angka1 = parseFloat(document.getElementById("angka1").value);
    const hasil = hitungAkar(angka1, "akar");
    document.getElementById("hasil-kalkulator").innerHTML = 
